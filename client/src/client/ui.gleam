...
fn view_spell_tooltip_content(s: spell.Spell) -> Element(Msg) {
  case s {
    spell.DamageSpell(kind:, ..) -> view_damage_spell_tooltip(kind)
    spell.ModifierSpell(kind:, ..) -> view_modifier_spell_tooltip(kind)
    spell.MulticastSpell(kind:, ..) -> view_multicast_spell_tooltip(kind)
  }
}
