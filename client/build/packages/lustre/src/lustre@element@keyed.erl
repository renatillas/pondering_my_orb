...
-spec do_extract_keyed_children(
    list({binary(), lustre@vdom@vnode:element(XKQ)}),
    lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(XKQ)),
    list(lustre@vdom@vnode:element(XKQ))
) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(XKQ)),
    list(lustre@vdom@vnode:element(XKQ))}.
...
-spec extract_keyed_children(list({binary(), lustre@vdom@vnode:element(XKI)})) -> {lustre@internals@mutable_map:mutable_map(binary(), lustre@vdom@vnode:element(XKI)),
    list(lustre@vdom@vnode:element(XKI))}.
...
-spec element(
    binary(),
    list(lustre@vdom@vattr:attribute(XIO)),
    list({binary(), lustre@vdom@vnode:element(XIO)})
) -> lustre@vdom@vnode:element(XIO).
...
-spec namespaced(
    binary(),
    binary(),
    list(lustre@vdom@vattr:attribute(XIU)),
    list({binary(), lustre@vdom@vnode:element(XIU)})
) -> lustre@vdom@vnode:element(XIU).
...
-spec fragment(list({binary(), lustre@vdom@vnode:element(XJA)})) -> lustre@vdom@vnode:element(XJA).
fragment(Children) ->
    {Keyed_children, Children@1} = extract_keyed_children(Children),
...
-spec ul(
    list(lustre@vdom@vattr:attribute(XJE)),
    list({binary(), lustre@vdom@vnode:element(XJE)})
) -> lustre@vdom@vnode:element(XJE).
ul(Attributes, Children) ->
    element(<<"ul"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 145).
-spec ol(
    list(lustre@vdom@vattr:attribute(XJK)),
    list({binary(), lustre@vdom@vnode:element(XJK)})
) -> lustre@vdom@vnode:element(XJK).
ol(Attributes, Children) ->
    element(<<"ol"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 152).
-spec 'div'(
    list(lustre@vdom@vattr:attribute(XJQ)),
    list({binary(), lustre@vdom@vnode:element(XJQ)})
) -> lustre@vdom@vnode:element(XJQ).
'div'(Attributes, Children) ->
    element(<<"div"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 159).
-spec tbody(
    list(lustre@vdom@vattr:attribute(XJW)),
    list({binary(), lustre@vdom@vnode:element(XJW)})
) -> lustre@vdom@vnode:element(XJW).
tbody(Attributes, Children) ->
    element(<<"tbody"/utf8>>, Attributes, Children).

-file("src/lustre/element/keyed.gleam", 166).
-spec dl(
    list(lustre@vdom@vattr:attribute(XKC)),
    list({binary(), lustre@vdom@vnode:element(XKC)})
) -> lustre@vdom@vnode:element(XKC).
