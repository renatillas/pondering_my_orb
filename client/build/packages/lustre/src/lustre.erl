...
-export([component/4, application/3, element/1, simple/3, named/2, supervised/2, factory/1, register/2, send/2, dispatch/1, shutdown/0, is_browser/0, start/3, is_registered/1, start_server_component/2]).
...
-opaque app(WAZ, WBA, WBB) :: {app,
        gleam@option:option(gleam@erlang@process:name(lustre@runtime@server@runtime:message(WBB))),
        fun((WAZ) -> {WBA, lustre@effect:effect(WBB)}),
        fun((WBA, WBB) -> {WBA, lustre@effect:effect(WBB)}),
        fun((WBA) -> lustre@vdom@vnode:element(WBB)),
        lustre@component:config(WBB)}.
...
-type runtime(WBC) :: any() | {gleam_phantom, WBC}.

-file("src/lustre.gleam", 313).
?DOC(
...
-spec component(
    fun((WCB) -> {WCC, lustre@effect:effect(WCD)}),
    fun((WCC, WCD) -> {WCC, lustre@effect:effect(WCD)}),
    fun((WCC) -> lustre@vdom@vnode:element(WCD)),
    list(lustre@component:option(WCD))
) -> app(WCB, WCC, WCD).
component(Init, Update, View, Options) ->
    {app, none, Init, Update, View, lustre@component:new(Options)}.

-file("src/lustre.gleam", 285).
?DOC(
...
-spec application(
    fun((WBS) -> {WBT, lustre@effect:effect(WBU)}),
    fun((WBT, WBU) -> {WBT, lustre@effect:effect(WBU)}),
    fun((WBT) -> lustre@vdom@vnode:element(WBU))
) -> app(WBS, WBT, WBU).
application(Init, Update, View) ->
    {app, none, Init, Update, View, (lustre@component:new([]))}.

-file("src/lustre.gleam", 250).
?DOC(
...
-spec element(lustre@vdom@vnode:element(WBF)) -> app(any(), nil, WBF).
element(View) ->
    application(
...
-spec simple(
    fun((WBL) -> WBM),
    fun((WBM, WBN) -> WBM),
    fun((WBM) -> lustre@vdom@vnode:element(WBN))
) -> app(WBL, WBM, WBN).
...
-spec named(
    app(WCM, WCN, WCO),
    gleam@erlang@process:name(lustre@runtime@server@runtime:message(WCO))
) -> app(WCM, WCN, WCO).
named(App, Name) ->
    {app,
        {some, Name},
...
-file("src/lustre.gleam", 367).
-spec do_start(app(WDG, any(), WDI), binary(), WDG) -> {ok, runtime(WDI)} |
    {error, error()}.
do_start(_, _, _) ->
    {error, not_a_browser}.

-file("src/lustre.gleam", 422).
?DOC(
...
-spec supervised(app(WDY, any(), WEA), WDY) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(lustre@runtime@server@runtime:message(WEA))).
supervised(App, Start_arguments) ->
    gleam@otp@supervision:worker(
        fun() ->
            lustre@runtime@server@runtime:start(
                erlang:element(2, App),
                (erlang:element(3, App))(Start_arguments),
                erlang:element(4, App),
                erlang:element(5, App),
                lustre@component:to_server_component_config(
                    erlang:element(6, App)
                )
            )
        end
...
-spec factory(app(WEH, any(), WEJ)) -> gleam@otp@factory_supervisor:builder(WEH, gleam@erlang@process:subject(lustre@runtime@server@runtime:message(WEJ))).
factory(App) ->
...
-spec send(runtime(WEY), lustre@runtime@server@runtime:message(WEY)) -> nil.
send(Runtime, Message) ->
    gleam@erlang@process:send(Runtime, Message).

-file("src/lustre.gleam", 501).
?DOC(
...
-spec dispatch(WFB) -> lustre@runtime@server@runtime:message(WFB).
dispatch(Msg) ->
    {effect_dispatched_message, Msg}.

-file("src/lustre.gleam", 510).
?DOC(
...
-spec start(app(WCX, any(), WCZ), binary(), WCX) -> {ok, runtime(WCZ)} |
    {error, error()}.
...
-spec start_server_component(app(WDP, any(), WDR), WDP) -> {ok, runtime(WDR)} |
    {error, error()}.
start_server_component(App, Start_args) ->
    Result = lustre@runtime@server@runtime:start(
        erlang:element(2, App),
        (erlang:element(3, App))(Start_args),
        erlang:element(4, App),
        erlang:element(5, App),
        lustre@component:to_server_component_config(erlang:element(6, App))
    ),
    case Result of
