...
-file("src/lustre/runtime/server/runtime.gleam", 372).
?DOC(false).
-spec handle_attribute_change(
    gleam@dict:dict(binary(), fun((binary()) -> {ok, ULO} | {error, nil})),
    binary(),
    binary()
) -> {ok, ULO} | {error, nil}.
...
-file("src/lustre/runtime/server/runtime.gleam", 384).
?DOC(false).
-spec handle_property_change(
    gleam@dict:dict(binary(), gleam@dynamic@decode:decoder(ULV)),
    binary(),
...
-file("src/lustre/runtime/server/runtime.gleam", 396).
?DOC(false).
-spec handle_effect(
    gleam@erlang@process:subject(message(UMB)),
    lustre@effect:effect(UMB)
) -> nil.
...
-file("src/lustre/runtime/server/runtime.gleam", 303).
?DOC(false).
...
?DOC(false).
-spec broadcast(
    gleam@dict:dict(gleam@erlang@process:subject(lustre@runtime@transport:client_message(UMF)), gleam@erlang@process:monitor()),
    gleam@set:set(fun((lustre@runtime@transport:client_message(UMF)) -> nil)),
    lustre@runtime@transport:client_message(UMF)
) -> nil.
...
-file("src/lustre/runtime/server/runtime.gleam", 150).
?DOC(false).
-spec loop(state(UKY, UKZ), message(UKZ)) -> gleam@otp@actor:next(state(UKY, UKZ), message(UKZ)).
loop(State, Message) ->
    case Message of
        {client_dispatched_message, Message@1} ->
            ...
            Msg = lustre@runtime@transport:reconcile(
        ...
        {effect_dispatched_message, Message@2} ->
            ...
            handle_effect(erlang:element(2, State), Effect),
            Msg@1 = lustre@runtime@transport:reconcile(
...
-file("src/lustre/runtime/server/runtime.gleam", 70).
?DOC(false).
-spec start(
    gleam@option:option(gleam@erlang@process:name(message(UKK))),
    {UKO, lustre@effect:effect(UKK)},
    fun((UKO, UKK) -> {UKO, lustre@effect:effect(UKK)}),
    fun((UKO) -> lustre@vdom@vnode:element(UKK)),
    config(UKK)
) -> {ok, gleam@otp@actor:started(gleam@erlang@process:subject(message(UKK)))} |
    {error, gleam@otp@actor:start_error()}.
start(Name, Init, Update, View, Config) ->
