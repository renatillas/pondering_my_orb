...
-spec from_tuple({DQA, DQA}) -> vec2(DQA).
from_tuple(Tuple) ->
...
-spec to_tuple(vec2(DQC)) -> {DQC, DQC}.
to_tuple(Vector) ->
...
-spec to_list(vec2(DQE)) -> list(DQE).
to_list(Vector) ->
    [erlang:element(2, Vector), erlang:element(3, Vector)].
...
-spec replace_x(vec2(DQL), DQL) -> vec2(DQL).
...
-spec replace_y(vec2(DQO), DQO) -> vec2(DQO).
...
-spec map_x(vec2(DQR), fun((DQR) -> DQR)) -> vec2(DQR).
map_x(Vector, Fun) ->
...
-spec map_y(vec2(DQU), fun((DQU) -> DQU)) -> vec2(DQU).
map_y(Vector, Fun) ->
...
-spec map(vec2(DRA), fun((DRA) -> DRC)) -> vec2(DRC).
map(Vector, Fun) ->
...
-spec map2(vec2(DRE), vec2(DRG), fun((DRE, DRG) -> DRI)) -> vec2(DRI).
map2(A, B, Fun) ->
    {vec2,
...
-spec result(vec2({ok, DRK} | {error, DRL})) -> {ok, vec2(DRK)} | {error, DRL}.
result(Vector) ->
    case Vector of
        {vec2, {ok, X}, {ok, Y}} ->
            {ok, {vec2, X, Y}};
