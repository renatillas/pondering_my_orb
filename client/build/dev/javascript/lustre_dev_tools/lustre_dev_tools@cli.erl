...
-spec string(
    ...
    fun((fun((glint:flags()) -> {ok, binary()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AFSL))
) -> glint:command(AFSL).
...
-spec flag(
    ...
    list(binary()),
    lustre_dev_tools@project:project(),
    fun((binary()) -> glint:flag(AFTC)),
    fun((gleam@dict:dict(binary(), tom:toml()), list(binary())) -> {ok, AFTC} |
        {error, tom:get_error()}),
    binary(),
    fun((fun((glint:flags()) -> {ok, AFTC} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AFTL))
) -> glint:command(AFTL).
...
-spec bool(
    ...
    fun((fun((glint:flags()) -> {ok, boolean()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AFSF))
) -> glint:command(AFSF).
...
-spec string_list(
    ...
    fun((fun((glint:flags()) -> {ok, list(binary())} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AFSS))
) -> glint:command(AFSS).
...
-spec int(
    ...
    fun((fun((glint:flags()) -> {ok, integer()} |
        {error, lustre_dev_tools@error:error()})) -> glint:command(AFSY))
) -> glint:command(AFSY).
