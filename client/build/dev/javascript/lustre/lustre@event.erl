...
-spec on(binary(), gleam@dynamic@decode:decoder(YJP)) -> lustre@vdom@vattr:attribute(YJP).
on(Name, Handler) ->
...
-spec advanced(
    binary(),
    gleam@dynamic@decode:decoder(lustre@vdom@vattr:handler(YJS))
) -> lustre@vdom@vattr:attribute(YJS).
...
-spec handler(YJW, boolean(), boolean()) -> lustre@vdom@vattr:handler(YJW).
...
-spec prevent_default(lustre@vdom@vattr:attribute(YJY)) -> lustre@vdom@vattr:attribute(YJY).
...
-spec stop_propagation(lustre@vdom@vattr:attribute(YKB)) -> lustre@vdom@vattr:attribute(YKB).
...
-spec debounce(lustre@vdom@vattr:attribute(YKE), integer()) -> lustre@vdom@vattr:attribute(YKE).
...
-spec throttle(lustre@vdom@vattr:attribute(YKH), integer()) -> lustre@vdom@vattr:attribute(YKH).
...
-spec on_keypress(fun((binary()) -> YKY)) -> lustre@vdom@vattr:attribute(YKY).
on_keypress(Msg) ->
    on(
        <<"keypress"/utf8>>,
        begin
...
-spec on_keydown(fun((binary()) -> YLA)) -> lustre@vdom@vattr:attribute(YLA).
on_keydown(Msg) ->
    on(
        <<"keydown"/utf8>>,
        begin
...
-spec on_keyup(fun((binary()) -> YLC)) -> lustre@vdom@vattr:attribute(YLC).
on_keyup(Msg) ->
    on(
        <<"keyup"/utf8>>,
        begin
...
-spec on_input(fun((binary()) -> YLE)) -> lustre@vdom@vattr:attribute(YLE).
on_input(Msg) ->
    on(
        <<"input"/utf8>>,
        begin
...
-spec on_change(fun((binary()) -> YLG)) -> lustre@vdom@vattr:attribute(YLG).
on_change(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_check(fun((boolean()) -> YLI)) -> lustre@vdom@vattr:attribute(YLI).
on_check(Msg) ->
    on(
        <<"change"/utf8>>,
        begin
...
-spec on_submit(fun((list({binary(), binary()})) -> YLL)) -> lustre@vdom@vattr:attribute(YLL).
on_submit(Msg) ->
    _pipe@2 = on(
        <<"submit"/utf8>>,
        begin
...
-spec on_focus(YLP) -> lustre@vdom@vattr:attribute(YLP).
on_focus(Msg) ->
    on(<<"focus"/utf8>>, gleam@dynamic@decode:success(Msg)).

-file("src/lustre/event.gleam", 352).
-spec on_blur(YLR) -> lustre@vdom@vattr:attribute(YLR).
on_blur(Msg) ->
