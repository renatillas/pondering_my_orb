...
-opaque config(VQY) :: {config,
        boolean(),
        boolean(),
        boolean(),
        list({binary(), fun((binary()) -> {ok, VQY} | {error, nil})}),
        list({binary(), gleam@dynamic@decode:decoder(VQY)}),
        list({binary(), gleam@dynamic@decode:decoder(VQY)}),
        boolean(),
        gleam@option:option(fun((binary()) -> VQY)),
        gleam@option:option(VQY),
        gleam@option:option(fun((binary()) -> VQY))}.

-opaque option(VQZ) :: {option, fun((config(VQZ)) -> config(VQZ))}.

-file("src/lustre/component.gleam", 133).
?DOC(false).
-spec new(list(option(VRA))) -> config(VRA).
new(Options) ->
...
-spec on_attribute_change(binary(), fun((binary()) -> {ok, VRE} | {error, nil})) -> option(VRE).
...
-spec on_property_change(binary(), gleam@dynamic@decode:decoder(VRI)) -> option(VRI).
...
-spec on_context_change(binary(), gleam@dynamic@decode:decoder(VRL)) -> option(VRL).
...
-spec on_form_autofill(fun((binary()) -> VRQ)) -> option(VRQ).
...
-spec on_form_reset(VRS) -> option(VRS).
on_form_reset(Msg) ->
    {option,
        fun(Config) ->
            {config,
...
-spec on_form_restore(fun((binary()) -> VRU)) -> option(VRU).
...
?DOC(false).
-spec to_server_component_config(config(VSC)) -> lustre@runtime@server@runtime:config(VSC).
...
-spec default_slot(
    list(lustre@vdom@vattr:attribute(VSF)),
    list(lustre@vdom@vnode:element(VSF))
) -> lustre@vdom@vnode:element(VSF).
...
-spec named_slot(
    binary(),
    list(lustre@vdom@vattr:attribute(VSL)),
    list(lustre@vdom@vnode:element(VSL))
) -> lustre@vdom@vnode:element(VSL).
