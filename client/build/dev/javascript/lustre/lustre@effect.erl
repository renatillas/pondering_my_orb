...
-opaque effect(QSQ) :: {effect,
        list(fun((actions(QSQ)) -> nil)),
        list(fun((actions(QSQ)) -> nil)),
        list(fun((actions(QSQ)) -> nil))}.

-type actions(QSR) :: {actions,
        fun((QSR) -> nil),
        fun((binary(), gleam@json:json()) -> nil),
        fun((gleam@erlang@process:selector(QSR)) -> nil),
...
-spec do_comap_select(
    actions(QUB),
    gleam@erlang@process:selector(QUD),
    fun((QUD) -> QUB)
) -> nil.
...
-spec do_comap_actions(actions(QTX), fun((QTZ) -> QTX)) -> actions(QTZ).
...
-spec do_map(list(fun((actions(QTR)) -> nil)), fun((QTR) -> QTU)) -> list(fun((actions(QTU)) -> nil)).
do_map(Effects, F) ->
    gleam@list:map(
        Effects,
        fun(Effect) ->
...
-spec map(effect(QTN), fun((QTN) -> QTP)) -> effect(QTP).
map(Effect, F) ->
    {effect,
...
-spec perform(
    ...
    fun((QUF) -> nil),
    fun((binary(), gleam@json:json()) -> nil),
    fun((gleam@erlang@process:selector(QUF)) -> nil),
    fun(() -> gleam@dynamic:dynamic_()),
    fun((binary(), gleam@json:json()) -> nil)
) -> nil.
...
-spec from(fun((fun((QSU) -> nil)) -> nil)) -> effect(QSU).
from(Effect) ->
    Task = fun(Actions) ->
...
-spec before_paint(fun((fun((QSW) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(QSW).
before_paint(Effect) ->
    Task = fun(Actions) ->
...
-spec after_paint(fun((fun((QSY) -> nil), gleam@dynamic:dynamic_()) -> nil)) -> effect(QSY).
after_paint(Effect) ->
    Task = fun(Actions) ->
        Root = (erlang:element(5, Actions))(),
        Dispatch = erlang:element(2, Actions),
...
?DOC(false).
-spec select(
    fun((fun((QTC) -> nil), gleam@erlang@process:subject(any())) -> gleam@erlang@process:selector(QTC))
) -> effect(QTC).
select(Sel) ->
    Task = fun(Actions) ->
...
-spec batch(list(effect(QTJ))) -> effect(QTJ).
batch(Effects) ->
    gleam@list:fold(
        Effects,
        {effect, [], [], []},
