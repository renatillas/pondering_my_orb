...
-type valid_message(MHZ) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, MHZ}.

-type websocket_message(MIA) :: {valid, valid_message(MIA)} | invalid.
...
-type handler_message(MIB) :: {internal, gramps@websocket:frame()} | {user, MIB}.

-type websocket_state(MIC) :: {websocket_state,
        bitstring(),
        MIC,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(MJI))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(MJI))).
...
-spec apply_frames(
    ...
    fun((MIY, handler_message(MIZ), websocket_connection()) -> mist@internal@next:next(MIY, MIZ)),
    websocket_connection(),
    mist@internal@next:next(MIY, websocket_message(MIZ)),
    fun((MIY) -> nil)
) -> mist@internal@next:next(MIY, websocket_message(MIZ)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {MIL,
        gleam@option:option(gleam@erlang@process:selector(MIM))}),
    fun((MIL) -> nil),
    fun((MIL, handler_message(MIM), websocket_connection()) -> mist@internal@next:next(MIL, MIM)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(MIM)))} |
    {error, nil}.
