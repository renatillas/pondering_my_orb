...
-opaque next(MWJ, MWK) :: {continue,
        MWJ,
        gleam@option:option(gleam@erlang@process:selector(MWK))} |
    normal_stop |
...
-opaque builder(MWL, MWM) :: {builder,
        integer(),
        fun((gleam@http@request:request(MWL)) -> gleam@http@response:response(MWM)),
...
-type websocket_message(MWN) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, MWN}.
...
-spec continue(MWO) -> next(MWO, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 58).
-spec with_selector(next(MWS, MWT), gleam@erlang@process:selector(MWT)) -> next(MWS, MWT).
...
-spec convert_next(next(MXH, MXI)) -> mist@internal@next:next(MXH, MXI).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(MYJ)) -> gleam@http@response:response(MYL))
) -> builder(MYJ, MYL).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(MYP, MYQ), integer()) -> builder(MYP, MYQ).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), MYV),
    integer(),
    gleam@http@response:response(MYV)
) -> builder(mist@internal@http:connection(), MYV).
...
-spec bind(builder(MZH, MZI), binary()) -> builder(MZH, MZI).
...
-spec with_ipv6(builder(MZN, MZO)) -> builder(MZN, MZO).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(MZT, MZU), binary(), binary()) -> builder(MZT, MZU).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(NAJ)
) -> {ok, websocket_message(NAJ)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((NAP, websocket_message(NAQ), mist@internal@websocket:websocket_connection()) -> next(NAP, NAQ)),
    fun((mist@internal@websocket:websocket_connection()) -> {NAP,
        gleam@option:option(gleam@erlang@process:selector(NAQ))}),
    fun((NAP) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(NBE)) -> {ok,
            gleam@otp@actor:initialised(NBG, NBE, any())} |
        {error, binary()}),
    fun((NBG, NBE, s_s_e_connection()) -> gleam@otp@actor:next(NBG, NBE))
) -> gleam@http@response:response(response_data()).
