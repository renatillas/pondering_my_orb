...
// .wrangler/tmp/bundle-kz7wls/checked-fetch.js
...
__name(console_log, "console_log");
...
__name(get7, "get");
function update(shift, node, index5, fun) {
  if (node instanceof Balanced) {
    let size$1 = node.size;
    let children = node.children;
    let node_index = bsr(index5, shift);
...
__name(append_node, "append_node");
function new$4() {
  return new Builder(toList([]), empty3(), append_node, append3);
}
__name(new$4, "new$");
...
__name(compress_nodes, "compress_nodes");
...
__name(initialise_loop, "initialise_loop");
function initialise(length6, fun) {
  let $ = initialise_loop(0, length6, new$4(), fun);
  if ($ instanceof Ok) {
    let shift = $[0][0];
    let nodes = $[0][1];
...
__name(get8, "get");
function update2(array4, index5, fun) {
  if (array4 instanceof Empty2) {
    return error_nil;
  } else {
    let shift = array4.shift;
    ...
    let $ = 0 <= index5 && index5 < size3(root);
    if ($) {
      return new Ok(new Array2(shift, update(shift, root, index5, fun)));
    } else {
      return error_nil;
    }
  }
...
function set(array4, index5, item) {
  return update2(array4, index5, (_) => {
    return item;
  });
}
__name(set, "set");
...
// build/dev/javascript/vec/vec/vec2.mjs
var Vec2 = class extends CustomType {
  static {
    __name(this, "Vec2");
  }
  constructor(x, y) {
...
var Spark = class extends CustomType {
  static {
    __name(this, "Spark");
  }
};
...
var NoEffect = class extends CustomType {
  static {
    __name(this, "NoEffect");
  }
};
var StaticSprite = class extends CustomType {
  static {
    __name(this, "StaticSprite");
  }
  constructor(texture_path, size5) {
    super();
    this.texture_path = texture_path;
    this.size = size5;
  }
};
var SpellVisuals = class extends CustomType {
  static {
    __name(this, "SpellVisuals");
  }
...
var Damage = class extends CustomType {
  ...
  constructor(name2, mana_cost, damage, projectile_speed, projectile_lifetime, projectile_size, cast_delay_addition, critical_chance, spread, visuals, has_trigger) {
    super();
    this.name = name2;
...
function new$7(name2, slot_count, max_mana, mana_recharge_rate, cast_delay, recharge_time, spells_per_cast, spread) {
  return new Wand(
    name2,
    repeat2(new None(), slot_count),
    max_mana,
    max_mana,
    mana_recharge_rate,
    cast_delay,
...
__name(new$7, "new$");
function set_spell(wand, slot_index, spell) {
  let $ = get8(wand.slots, slot_index);
  if ($ instanceof Ok) {
    let $1 = $[0];
    if ($1 instanceof Some) {
      return new Error2(void 0);
      ...
      return map4(
        set(wand.slots, slot_index, new Some(spell)),
        (slots) => {
          return new Wand(
            wand.name,
            slots,
...
function new$8(id4, name2, position) {
  return new Player(
    id4,
    name2,
    position,
...
var GameState = class extends CustomType {
  ...
  constructor(tick2, players, projectiles, enemies, next_projectile_id, next_enemy_id) {
    super();
    this.tick = tick2;
    this.players = players;
    this.projectiles = projectiles;
...
function new$9() {
  return new GameState(0, make(), make(), make(), 0, 0);
}
__name(new$9, "new$");
...
function process_player_action(players, events, player_id, player_state, action, next_projectile_id) {
  if (action instanceof None2) {
    return [players, events, next_projectile_id];
    ...
    return [new_players, events, next_projectile_id];
    ...
    if (wand_option instanceof Some) {
      ...
      if (cast_result instanceof CastSuccess) {
        ...
        let $1 = fold2(
          projectiles,
          [toList([]), next_projectile_id],
          (acc, spell_projectile) => {
            let event_list;
            let current_id;
            event_list = acc[0];
            current_id = acc[1];
            ...
            let event = new ProjectileCreated(projectile);
            return [append2(event_list, toList([event])), current_id + 1];
          }
        );
        let new_events;
        let final_id;
        new_events = $1[0];
        final_id = $1[1];
        return [new_players, append2(events, new_events), final_id];
      } else {
        return [players, events, next_projectile_id];
      }
    } else {
...
function process_player_inputs(game_state, player_inputs, next_projectile_id) {
  let $ = fold(
    player_inputs,
    [game_state.players, toList([]), next_projectile_id],
  ...
  let new_players;
  let events;
  let final_projectile_id;
  new_players = $[0];
  events = $[1];
  final_projectile_id = $[2];
  return [
    new GameState(
      game_state.tick,
      new_players,
...
function handle_leave(room_state, player_id) {
  ...
  let new_game_players = delete$(room_state.game_state.players, player_id);
  let _block;
  let _record = room_state.game_state;
  _block = new GameState(
    _record.tick,
    new_game_players,
    _record.projectiles,
    _record.enemies,
    _record.next_projectile_id,
    _record.next_enemy_id
  );
  let new_game_state = _block;
...
function websocket_close2(_, ws, _1, _2, _3, room_state) {
  ...
  if ($ instanceof Ok) {
    ...
    if ($1 instanceof Some) {
      ...
      pid = player_id[0];
      console_log(
        "[GameRoom] Player " + to_string(pid) + " disconnected"
      );
      let new_players = delete$(room_state.players, player_id);
      let new_game_players = delete$(
        room_state.game_state.players,
        player_id
      );
      let _block;
      let _record = room_state.game_state;
...
function give_starter_wand(new_player) {
  let starter_wand = new$7(
    "Starter Wand",
    3,
    100,
    10,
  ...
  let $ = set_spell(starter_wand, 0, spark());
  let starter_wand$1;
  if ($ instanceof Ok) {
    starter_wand$1 = $[0];
  } else {
  ...
  let $1 = set_spell(starter_wand$1, 1, spark());
  let starter_wand$2;
  if ($1 instanceof Ok) {
    starter_wand$2 = $1[0];
  } else {
  ...
  let $2 = set_spell(starter_wand$2, 2, spark());
  let starter_wand$3;
  if ($2 instanceof Ok) {
    starter_wand$3 = $2[0];
  } else {
  ...
  let new_wands = new WandInventory(
    new Some(starter_wand$3),
    new None(),
    new None(),
    new None()
  );
  return new Player(
    ...
    0,
    new_wands,
    new_player.movement_state
  );
}
__name(give_starter_wand, "give_starter_wand");
function handle_join(state, room_state, player_id, ws, _, player_name) {
  ...
  let _block;
  let _pipe = new$8(player_id, player_name, new Vec3(0, 0.9, 0));
  _block = give_starter_wand(_pipe);
  let new_player = _block;
  let player_info = new PlayerInfo(new_player, ws);
  let _block$1;
  let _pipe$1 = values(room_state.players);
  _block$1 = map2(_pipe$1, (info) => {
    return info.state;
  });
  let existing_players = _block$1;
  let new_players = insert(room_state.players, player_id, player_info);
  let new_game_players = insert(
    room_state.game_state.players,
    player_id,
    new_player
  );
  ...
  let _block$3;
  let _pipe$2 = keys(room_state.players);
  let _pipe$3 = map2(_pipe$2, (pid) => {
    return [pid, broadcast_msg];
  });
  _block$3 = prepend2(_pipe$3, [player_id, join_msg]);
  let messages2 = _block$3;
  return [new_state, messages2];
}
...
// .wrangler/tmp/bundle-kz7wls/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-kz7wls/middleware-loader.entry.ts
