  ...
  case iv.get(wand.slots, wrapped_index) {
    ...
    Ok(option.Some(current_spell)) ->
      case current_spell {
        ...
        spell.DamageSpell(id:, ui_sprite:, kind:) ->
  ...
  case current >= end {
    ...
    False -> {
      ...
      let new_acc = case iv.get(slots, wrapped_index) {
        Ok(option.Some(spell.ModifierSpell(kind:, ..))) -> iv.append(acc, kind)
        _ -> acc
      }
...
fn find_next_damage_spell(
  slots: iv.Array(option.Option(spell.Spell)),
  start_index: Int,
) -> Result(#(spell.Id, String, spell.DamageSpell, Int), Nil) {
      ...
      case would_go_backwards {
        ...
        False -> {
          case iv.get(slots, wrapped_index) {
            Ok(option.Some(spell.DamageSpell(id:, kind:, ui_sprite:))) ->
              Ok(#(id, ui_sprite, kind, current_index))
...
fn build_trigger_payload(
  ...
  case find_next_damage_spell(slots, current_index + 1) {
    Ok(#(payload_id, ui_sprite, payload_spell, payload_index)) -> {
    ...
    Error(Nil) -> #(option.None, option.None)
  }
}
...
fn process_damage_spell(
  ...
  damage_spell spell: spell.DamageSpell,
  wrapped_index wrapped_index: Int,
  wrapped_flag wrapped_flag: Bool,
  state state: CastState,
  context context: CastContext,
) -> #(CastResult, Wand) {
  let modified =
    spell.apply_modifiers(id, ui_sprite, spell, state.accumulated_modifiers)
  let new_cast_delay =
  ...
  case
    check_mana_sufficient(wand, state.total_mana_used, modified.total_mana_cost)
  {
    Error(#(required, available)) -> #(
      NotEnoughMana(required: required, available: available),
      wand,
    ...
    Ok(new_mana_used) -> {
      let spread_direction =
        apply_spread(context.direction, wand.spread +. modified.final_spread)
