...
-spec be_ok({ok, EDI} | {error, any()}) -> EDI.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, EDN}) -> EDN.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(EDQ)) -> EDQ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
