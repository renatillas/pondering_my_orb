...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(DLC)) -> {ok, DLC} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(DKW)) -> {ok, DKW} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(DKS)) -> {ok, DKS} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(DLI), fun((DLI) -> json())) -> json().
...
-spec array(list(DLM), fun((DLM) -> json())) -> json().
