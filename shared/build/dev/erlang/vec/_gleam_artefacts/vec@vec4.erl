...
-type vec4(FTW) :: {vec4, FTW, FTW, FTW, FTW}.
...
-spec from_tuple({FUA, FUA, FUA, FUA}) -> vec4(FUA).
from_tuple(Tuple) ->
    {vec4,
...
-spec to_tuple(vec4(FUC)) -> {FUC, FUC, FUC, FUC}.
to_tuple(Vector) ->
...
-spec to_list(vec4(FUE)) -> list(FUE).
to_list(Vector) ->
    [erlang:element(2, Vector),
        erlang:element(3, Vector),
        erlang:element(4, Vector),
...
-spec replace_x(vec4(FUP), FUP) -> vec4(FUP).
...
-spec replace_y(vec4(FUS), FUS) -> vec4(FUS).
...
-spec replace_z(vec4(FUV), FUV) -> vec4(FUV).
...
-spec replace_w(vec4(FUY), FUY) -> vec4(FUY).
...
-spec map_x(vec4(FVB), fun((FVB) -> FVB)) -> vec4(FVB).
map_x(Vector, Fun) ->
    {vec4,
...
-spec map_y(vec4(FVE), fun((FVE) -> FVE)) -> vec4(FVE).
map_y(Vector, Fun) ->
    {vec4,
        erlang:element(2, Vector),
...
-spec map_z(vec4(FVH), fun((FVH) -> FVH)) -> vec4(FVH).
map_z(Vector, Fun) ->
    {vec4,
        erlang:element(2, Vector),
        erlang:element(3, Vector),
...
-spec map_w(vec4(FVK), fun((FVK) -> FVK)) -> vec4(FVK).
map_w(Vector, Fun) ->
    {vec4,
        erlang:element(2, Vector),
        erlang:element(3, Vector),
...
-spec swap_xy(vec4(FVN)) -> vec4(FVN).
swap_xy(Vector) ->
    {vec4,
        erlang:element(3, Vector),
        erlang:element(2, Vector),
...
-spec swap_xz(vec4(FVQ)) -> vec4(FVQ).
swap_xz(Vector) ->
    {vec4,
        erlang:element(4, Vector),
        erlang:element(3, Vector),
...
-spec swap_xw(vec4(FVT)) -> vec4(FVT).
swap_xw(Vector) ->
    {vec4,
        erlang:element(5, Vector),
        erlang:element(3, Vector),
...
-spec swap_yz(vec4(FVW)) -> vec4(FVW).
swap_yz(Vector) ->
    {vec4,
        erlang:element(2, Vector),
        erlang:element(4, Vector),
...
-spec swap_yw(vec4(FVZ)) -> vec4(FVZ).
swap_yw(Vector) ->
    {vec4,
        erlang:element(2, Vector),
        erlang:element(5, Vector),
...
-spec swap_zw(vec4(FWC)) -> vec4(FWC).
swap_zw(Vector) ->
    {vec4,
        erlang:element(2, Vector),
        erlang:element(3, Vector),
...
-spec map(vec4(FWF), fun((FWF) -> FWH)) -> vec4(FWH).
map(Vector, Fun) ->
    {vec4,
        Fun(erlang:element(2, Vector)),
        Fun(erlang:element(3, Vector)),
...
-spec map2(vec4(FWJ), vec4(FWL), fun((FWJ, FWL) -> FWN)) -> vec4(FWN).
map2(A, B, Fun) ->
    {vec4,
...
-spec result(vec4({ok, FWP} | {error, FWQ})) -> {ok, vec4(FWP)} | {error, FWQ}.
result(Vector) ->
    case Vector of
