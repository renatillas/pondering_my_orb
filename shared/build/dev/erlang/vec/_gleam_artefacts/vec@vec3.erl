...
-type vec3(EYE) :: {vec3, EYE, EYE, EYE}.
...
-spec from_tuple({EYI, EYI, EYI}) -> vec3(EYI).
from_tuple(Tuple) ->
    {vec3,
...
-spec to_tuple(vec3(EYK)) -> {EYK, EYK, EYK}.
to_tuple(Vector) ->
...
-spec to_list(vec3(EYM)) -> list(EYM).
to_list(Vector) ->
    [erlang:element(2, Vector),
        erlang:element(3, Vector),
        erlang:element(4, Vector)].
...
-spec replace_x(vec3(EYV), EYV) -> vec3(EYV).
...
-spec replace_y(vec3(EYY), EYY) -> vec3(EYY).
...
-spec replace_z(vec3(EZB), EZB) -> vec3(EZB).
...
-spec map_x(vec3(EZE), fun((EZE) -> EZE)) -> vec3(EZE).
map_x(Vector, Fun) ->
    {vec3,
...
-spec map_y(vec3(EZH), fun((EZH) -> EZH)) -> vec3(EZH).
map_y(Vector, Fun) ->
    {vec3,
        erlang:element(2, Vector),
...
-spec map_z(vec3(EZK), fun((EZK) -> EZK)) -> vec3(EZK).
map_z(Vector, Fun) ->
    {vec3,
        erlang:element(2, Vector),
        erlang:element(3, Vector),
...
-spec swap_xy(vec3(EZN)) -> vec3(EZN).
swap_xy(Vector) ->
    {vec3,
        erlang:element(3, Vector),
        erlang:element(2, Vector),
...
-spec swap_xz(vec3(EZQ)) -> vec3(EZQ).
swap_xz(Vector) ->
    {vec3,
        erlang:element(4, Vector),
        erlang:element(3, Vector),
...
-spec swap_yz(vec3(EZT)) -> vec3(EZT).
swap_yz(Vector) ->
    {vec3,
        erlang:element(2, Vector),
        erlang:element(4, Vector),
...
-spec map(vec3(EZW), fun((EZW) -> EZY)) -> vec3(EZY).
map(Vector, Fun) ->
    {vec3,
        Fun(erlang:element(2, Vector)),
        Fun(erlang:element(3, Vector)),
...
-spec map2(vec3(FAA), vec3(FAC), fun((FAA, FAC) -> FAE)) -> vec3(FAE).
map2(A, B, Fun) ->
    {vec3,
...
-spec result(vec3({ok, FAG} | {error, FAH})) -> {ok, vec3(FAG)} | {error, FAH}.
result(Vector) ->
    case Vector of
        {vec3, {ok, X}, {ok, Y}, {ok, Z}} ->
            {ok, {vec3, X, Y, Z}};
