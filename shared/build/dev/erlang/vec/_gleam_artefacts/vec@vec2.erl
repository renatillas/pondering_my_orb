...
-spec from_tuple({EFW, EFW}) -> vec2(EFW).
from_tuple(Tuple) ->
...
-spec to_tuple(vec2(EFY)) -> {EFY, EFY}.
to_tuple(Vector) ->
...
-spec to_list(vec2(EGA)) -> list(EGA).
to_list(Vector) ->
    [erlang:element(2, Vector), erlang:element(3, Vector)].
...
-spec replace_x(vec2(EGH), EGH) -> vec2(EGH).
...
-spec replace_y(vec2(EGK), EGK) -> vec2(EGK).
...
-spec map_x(vec2(EGN), fun((EGN) -> EGN)) -> vec2(EGN).
map_x(Vector, Fun) ->
...
-spec map_y(vec2(EGQ), fun((EGQ) -> EGQ)) -> vec2(EGQ).
map_y(Vector, Fun) ->
...
-spec map(vec2(EGW), fun((EGW) -> EGY)) -> vec2(EGY).
map(Vector, Fun) ->
...
-spec map2(vec2(EHA), vec2(EHC), fun((EHA, EHC) -> EHE)) -> vec2(EHE).
map2(A, B, Fun) ->
    {vec2,
...
-spec result(vec2({ok, EHG} | {error, EHH})) -> {ok, vec2(EHG)} | {error, EHH}.
result(Vector) ->
    case Vector of
        {vec2, {ok, X}, {ok, Y}} ->
            {ok, {vec2, X, Y}};
